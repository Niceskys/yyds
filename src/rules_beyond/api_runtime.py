...
# The production match now uses the standard combat values; pacing comes from
# risk-aware action choice instead of inflating HP or reducing weapon damage.
PLAYTEST_CONFIG = GameConfig()
...
def build_runtime_repository_from_env() -> InMemoryMatchRepository:
    ...
    if custom_mode:
        ...
        # One real, low-cost request verifies the endpoint, key, model and
        # OpenAI-compatible response shape before the game is exposed to players.
        custom_model_factory().check_connection()
        ...
        service_factory = MatchServiceFactory(
            ...
            model_name=values[DEFAULT_MODEL_API_MODEL_ENV],
            endpoint_url=values[DEFAULT_MODEL_API_BASE_URL_ENV],
        )

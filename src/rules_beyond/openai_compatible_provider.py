...
def ensure_safe_api_url(url: str, *, label: str = "API URL") -> str:
    """Accept a user-configured absolute HTTP(S) endpoint."""
        ...
        *,
        ...
        for attempt in range(self.max_attempts):
            ...
            try:
                ...
                if attempt + 1 < self.max_attempts and retryable:
        ...
        if raw is None:
            raise OpenAICompatibleProviderError("Model API request failed")
...
class OpenAICompatibleModel:
    ...
    def check_connection(self) -> None:
        """Verify network, authentication, model selection, and response shape."""

        raw = self.generate_strategy(
            system_prompt=(
                'This is a connection check. Return exactly {"intent":"HOLD"} '
                "and no other text."
            ),
            observation="{}",
        )
        try:
            ...
            raise OpenAICompatibleProviderError(
                "Model API connection check returned non-JSON content"
            ) from exc
        if (
            not isinstance(decoded, dict)
            or set(decoded) != {"intent"}
            or decoded.get("intent") not in {"PRESSURE", "KITE", "EVADE", "HOLD"}
        ):
            raise OpenAICompatibleProviderError(
                "Model API connection check returned an incompatible strategy response"
            )

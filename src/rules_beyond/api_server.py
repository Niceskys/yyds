...
The MiMo repository/provider wiring is built inside ``create_runtime_app()`` at
startup, so importing this module alone never requires ``MIMO_API_KEY`` and never
...
def create_runtime_app() -> FastAPI:
    """FastAPI app bound to the real MiMo-backed in-memory repository."""

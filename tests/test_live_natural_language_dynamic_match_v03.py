...
from rules_beyond.live_natural_language_dynamic_match_v03 import (
    LIVE_MATCH_SEEDS,
    LIVE_NL_SCHEDULE_V03,
    MemoizedRuleCandidateModel,
    build_live_controller_v03,
    evaluate_v03_gate,
    play_live_match_v03,
)
...
def test_memoized_model_returns_one_provider_decision_per_identical_request() -> None:
    ...
    player_text = LIVE_NL_SCHEDULE_V03[1]
...
def test_v03_offline_gate_compiles_each_semantic_request_once_across_combat_seeds() -> None:
    underlying = CountingGateOracleModel()
    cached = MemoizedRuleCandidateModel(underlying)

    traces = tuple(
        play_live_match_v03(
            build_live_controller_v03(cached),  # type: ignore[arg-type]
            seed=seed,
    ...
    assert evaluate_v03_gate(traces) == ()

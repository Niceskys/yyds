...
function buildStatsViewModel(stats: EffectiveStatsPublicView): StatViewModel[] {
  const cooldown =
    stats.cooldown_weapons.length > 0
      ? stats.cooldown_weapons.map((weapon) => labelWeapon(weapon)).join('、')
      : '无';
  return [
...
export function buildEscalationViewModel(
  ...
  } else if (escalation.rounds_until_next_level === null) {
    hint = '暂无进一步升温信息。';
  } else if (escalation.rounds_until_next_level <= 0) {
    hint = '下一次无伤害结算后，战局将继续升温。';
  } else {
    hint = `再持续 ${escalation.rounds_until_next_level} 回合无伤害，战局将进一步升温。`;
  }
  return {
    level: escalation.level,
    levelLabel,
    noDamageStreak: escalation.no_damage_streak,
    nextLevelAt: escalation.next_level_at_no_damage,
    roundsUntilNextLevel: escalation.rounds_until_next_level,
    hint,
  };
}
...
export function buildRuleViewModel(rule: RulePublicView | null): RuleViewModel | null {
  ...
  return { ruleId: rule.rule_id, playerText: rule.player_text, effectSummary };
}
...
export function buildActionViewModel(action: ActionPublicView | null): ActionViewModel {
  ...
  const moveLabel =
    action.move_path.length > 0
      ? `移动 ${action.move_path.map(labelDirection).join(' → ')}`
      : '未移动';
...
export function buildDecisionViewModel(
  ...
  return {
    afterRound: decision.after_round,
...
export function buildRuleFeedbackViewModel(
  ...
  return {
    ...
    suggestedRephrase: result.suggested_rephrase,
